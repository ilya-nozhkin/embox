...
int pwm_add(uint8_t pin){
	...
	pwm.gpio = gpio_by_num(pin);
	pwm.power = 0;

	pwms[idx] = pwm;

	gpio_settings(pwm.gpio, 0, GPIO_MODE_OUTPUT);
...
int pwm_set_power(uint8_t idx, int power){
	if(idx >= NPWM)
		return -1;
...
void pwm_on(uint8_t idx){
	gpio_set_level(pwms[idx].gpio, 0, 1);
}

void pwm_off(uint8_t idx){
	gpio_set_level(pwms[idx].gpio, 0, 0);
}
